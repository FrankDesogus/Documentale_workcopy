...
Usare il comando dedicato (idempotente, legge i nomi direttamente dal
codice — `documents/permissions.py`, `ecn/permissions.py` — quindi non
può disallinearsi):
...
Crea tutti e 10 i gruppi applicativi (`Document Readers`, `Document
Authors`, `Document Approvers`, `Document Auditors`, `Document Managers`,
`Quality Manager`, `Quality Operator`, `Direction`, `ECN Proposers`,
`Change Control Board`) con una breve descrizione per ciascuno.
